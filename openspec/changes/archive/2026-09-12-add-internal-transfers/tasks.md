...
- [x] 6.1 Prueba manual: crear una transferencia entre dos cuentas existentes con datos reales de desarrollo, confirmar que aparecen dos movimientos vinculados, que los saldos de ambas cuentas se actualizan correctamente, que el KPI de ingresos/egresos del mes no se infla, y que intentar editar/eliminar una sola pata desde el listado general es rechazado. Migraciones aplicadas contra ambas bases reales (`DefaultConnection` y `Tenant_geocobre`); validado por el usuario con uso real de la funcionalidad (se detectaron y corrigieron dos issues durante esta validación, ver tareas 7.x).
...
## 7. Extensiones surgidas de la validación con uso real

- [x] 7.1 Corregir `categorias-movimiento.ts`: faltaba `ChangeDetectorRef.detectChanges()` tras recibir la respuesta del API, dejando la tabla de categorías vacía pese a que el endpoint devolvía datos (bug preexistente, detectado al usar el flujo de transferencias).
- [x] 7.2 Poner al día la base de datos del tenant `geocobre`, que tenía solo 3 de 26 migraciones aplicadas (causa raíz del punto anterior, no relacionada con el código de esta funcionalidad).
- [x] 7.3 Permitir, desde el formulario de edición de movimientos, cambiar el "Tipo de Movimiento" a "Transferencia entre Cuentas" también sobre un movimiento existente (sin `TransferGroupId`): borra el movimiento individual y crea la transferencia nueva en su lugar, con aviso previo en la UI (`movimiento-form.component.ts`).
- [x] 7.4 Agregar `IsSystemDefault` a `MovementCategory`: marcar las dos categorías de Transferencia Interna como reservadas del sistema, excluirlas del selector de categoría en la carga manual de Ingreso/Egreso, y bloquear su edición/eliminación en backend (`MovementCategoryEndpoints.cs`) y UI (`categorias-movimiento.ts`/`.html`/`.css`). Migración aplicada contra ambas bases reales.
